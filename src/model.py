...
class DinoMHC_IGeoReasonAblation(nn.Module):
    """
    Complete DinoMHC Architecture.
    ...
    def forward(
        ...
        if self.config['task_head'] == 'contact':
            ...
            # Calculate avg peptide and groove
            peptide_sum = (peptide_fused * peptide_core_mask.unsqueeze(-1)).sum(dim=1)  # [batch, dim]
            peptide_len = peptide_core_mask.sum(dim=1, keepdim=True).to(dtype=peptide_sum.dtype)  # [batch, 1]
            eps = torch.tensor(1e-8, device=peptide_sum.device, dtype=peptide_sum.dtype)
            peptide_pooled = peptide_sum / (peptide_len + eps)  # [batch, dim]
            
            groove_pooled = groove_fused.mean(dim=1)  # [batch, dim]
            
            combined_features = peptide_pooled + groove_pooled  # [batch, dim]
            
            outputs['prediction'] = self.task_head(combined_features, flank_features=flank_features)
...
class DinoMHC_FusionLayerAblation(nn.Module):
    """
    Complete DinoMHC Architecture.
